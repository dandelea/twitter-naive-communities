...
class Predictor:
    ...
    def __init__(self,
                 parametersPvsN=best_parametersPvsN,
                 parametersSvsNS=best_parametersSvsNS):
    ...
    def train(self, path="bot/data/trained_models.sav"):
        """
        Train the model and stores it at 'path' parameter.
        """
        ...
        if os.path.exists(path):
            ...
            self.modelPvsN = Pipeline([('tfidf', tfidf1), ('multinomialnaive', multinomibalnb1)])
            ...
            self.modelSvsNS = Pipeline([('tfidf', tfidf2), ('multinomialnaive', multinomibalnb2)])
    ...
    def predict(self, x):
        """
        Uses the trained model and predicts the classification for a tweet.
        Only works if the model is trained.
        """
        if self.trained:
...
def calculate_best_parameters(tweets, classifications, language):
    ...
    model = Pipeline([('tfidf', tfidf), ('multinomialnaive', multinomibalnb)])
